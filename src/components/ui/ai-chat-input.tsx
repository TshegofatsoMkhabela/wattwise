...
export const PromptInput = React.forwardRef<HTMLDivElement, PromptInputProps>(
  (
    ...
    const startRecording = useCallback(async () => {
      // Unsupported browser (e.g. Firefox) → tell the user instead of failing silently.
      ...
      // Insecure context (plain http) → getUserMedia is unavailable; explain why.
      if (!navigator.mediaDevices?.getUserMedia) {
        toast.error("Microphone unavailable — voice input needs a secure (HTTPS) connection.");
        return;
      }
      ...
      try {
        ...
        // Permission denied or no device — surface guidance and bail cleanly.
        toast.error("Microphone blocked — allow mic access to use voice.");
        return;
      }
      ...
      // Setup Web Audio API for the live waveform visualizer.
      ...
      // Setup Speech Recognition (browser dictation into the textarea). en-ZA biases
      // the recogniser toward South African English for better accent accuracy.
      ...
      recognition.onerror = (e) => {
        const { toastMessage, level } = describeSpeechError(e.error);
        if (toastMessage) {
          if (level === "error") toast.error(toastMessage);
          else toast.warning(toastMessage);
        }
        stopRecording();
      };
    ...
    return (
      <>
        ...
        <div
          ...
          onBlur={handleBlur}
          className={cn("relative flex flex-col w-full", disabled && "opacity-60 pointer-events-none", className)}
          style={{
        ...
        >
          ...
          >
            <style
              ...
              .prompt-scrollbar:hover::-webkit-scrollbar-thumb { background: rgba(100,116,139,0.35); }
            `,
              }}
            />
            ...
            {/* Listening cue — sits where the actions row vacates while recording. */}
            <div
              aria-hidden={!isRecording}
              className={cn(
                "absolute bottom-2 left-4 z-[10] flex h-7 items-center text-xs font-medium text-muted-foreground transition-all duration-300 ease-[cubic-bezier(0.175,0.885,0.32,1.275)]",
            ...
            <span className="sr-only" aria-live="polite">
              {isRecording ? "Listening for voice input" : ""}
            </span>

            {/* Audio Wave Visualizer Overlay positioned to the left of the mic button */}
            <div
              className={cn(
