  ...
  return (
    <AppLayout title={`Meter ${meterId}`}>
      {meter ? (
        ...
        <div className="text-sm text-slate-400">Meter not found.</div>
      )}
    </AppLayout>
  );
}
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <Link
            to="/municipality"
            className="inline-flex items-center gap-1 text-xs text-slate-400 hover:text-slate-700 mb-2"
          >
          ...
          <div className="mt-2 flex items-center gap-3">
            ...
            <span className="text-xs text-slate-400">
              Last seen {formatDistanceToNow(new Date(meter.lastSeenAt), { addSuffix: true })}
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MiniMetric
          ...
          value={
            <>
              <CountUp value={current.voltage} decimals={1} /> V
            </>
          }
          mono
        />
        <MiniMetric
          ...
          value={
            <>
              <CountUp value={todayKWh} decimals={2} /> kWh
            </>
          }
          mono
        />
        <MiniMetric
          label="Cost today"
          value={formatZAR(todayKWh * TARIFF_PER_KWH)}
      ...
      <Card>
        ...
        {alerts.length === 0 ? (
          <div className="text-xs text-slate-400 py-6 text-center">
            No tamper events recorded for this meter.
          </div>
        ) : (
              ...
              return (
                <div key={a.id} className={`border-l-4 ${border} bg-slate-50 rounded-r-lg p-3`}>
                  <div className="flex items-center gap-2">
                    <Icon
                      className={`w-4 h-4 flex-shrink-0 ${a.severity === "critical" ? "text-red-500" : "text-amber-500"}`}
                    />
                  ...
                  <div className="text-[11px] text-slate-400 mt-1 ml-6">
                    Deviation: +{a.deviationPct ?? 42}% ·{" "}
                    {a.assignedTo ? `Dispatched to ${a.assignedTo}` : "Not dispatched"}
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Last 30 days · <span className="text-red-400 font-medium">red bars</span> mark tamper
              days
            </p>
          </div>
        </div>
      ...
      <Card>
        ...
        <div className="grid sm:grid-cols-2 gap-x-6 gap-y-0 text-sm">
          ...
          <Info label="Contact">
            <span className="font-mono">{meter.consumerPhone}</span>
          </Info>
          ...
          <Info label="Baseline watts">
            {editBaseline ? (
              ...
              <button
                ...
                className="font-mono inline-flex items-center gap-1 text-slate-800 hover:text-[#005EB8] transition-colors"
              >
      ...
      <DispatchModal
        ...
        summary={
          meter.tamperEvents > 0
            ? `${meter.tamperEvents} active tamper event(s) require investigation.`
            : undefined
        }
      />
    </div>
...
function MiniMetric({
  ...
  return (
    <Card>
      ...
      <div
        className={`mt-1 text-xl font-bold ${mono ? "font-mono" : ""} ${accent || "text-slate-900"}`}
      >
        {value}
      </div>
    </Card>
