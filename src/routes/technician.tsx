...
function JobCard({ job }: { job: Job }) {
  ...
  const advance = async (status: Job["status"], statusLabel: string, toastTitle: string, note?: string) => {
    ...
    <Card>
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex gap-1.5">
          <ActionBtn onClick={() => advance("en_route", "En Route", "Marked en route")} disabled={curIndex >= 1}>Mark En Route</ActionBtn>
          <ActionBtn onClick={() => advance("on_site", "On Site", "Marked on site")} disabled={curIndex >= 2}>Mark On Site</ActionBtn>
          <ActionBtn onClick={() => advance("resolved", "Resolved", "Job resolved", "Resolved on site")} primary>Resolve</ActionBtn>
        </div>
      </div>
...
function QuickReportForm() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const res = await sendAgentTrigger(
      ...
      photo ? [{ contentType: "image/jpeg", contentUrl: photo.dataUrl, name: photo.name }] : undefined
    );
  ...
  return (
    <Card className="sticky top-20">
      ...
      <form onSubmit={submit} className="space-y-3">
        ...
        <button type="submit" disabled={submitting} className="w-full bg-[#005EB8] text-white font-semibold py-2.5 rounded-lg text-sm hover:bg-[#003F8A] transition-colors inline-flex items-center justify-center gap-2 disabled:opacity-60">
